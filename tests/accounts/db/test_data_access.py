...
def test_get_transaction(dal, mock_session):
    mock_session.query().filter_by().one_or_none.return_value = Transactions(id="1", transaction_description="Test Transaction")

    transaction = dal.get_transaction("1")
    assert transaction.transaction_description == "Test Transaction"

def test_list_transactions_for_book(dal, mock_session):
    mock_session.query().filter_by().all.return_value = [Transactions(id="1", transaction_description="Test Transaction")]
...
def test_update_transaction(dal, mock_session):
    mock_session.query().filter_by().one_or_none.return_value = Transactions(id="1", transaction_description="Old Description")
    mock_session.commit = MagicMock()
