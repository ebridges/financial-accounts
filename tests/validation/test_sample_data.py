...
class TestQifFileStructure:
    ...
    def test_sample_6063_structure(self, sample_data_dir):
        ...
        assert qif.transactions, "No transactions found in QIF file"
        assert (
            qif.account_info.get('N') == "Liabilities:Credit Cards:creditcard-chase-personal-6063"
        )
...
class TestBasicImport:
    ...
    def test_import_sample_1381(
        self, initialized_db_with_accounts, validation_book_name, sample_data_dir
    ):
...
class TestMatchingRules:
    ...
    def test_account_matchable_rules(
        self, initialized_db_with_accounts, validation_book_name, matching_config
    ):
...
class TestBalanceIntegrity:
    ...
    def test_all_transactions_balanced(
        self, initialized_db_with_accounts, validation_book_name, sample_data_dir, sample_qif_files
    ):
        ...
        with BookContext(validation_book_name, initialized_db_with_accounts) as ctx:
            ...
            assert (
                len(unbalanced) == 0
            ), f"Found {len(unbalanced)} unbalanced transactions: {unbalanced}"


class TestDuplicateDetection:
    """Tests for duplicate transaction detection."""

    def test_duplicate_detection_and_reporting(
        self, initialized_db_with_accounts, validation_book_name
    ):
        """
