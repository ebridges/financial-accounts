...
COMPREHENSIVE_FILES = ["samples-1381.qif", "samples-1605.qif", "samples-6063.qif"]
...
class TestComprehensiveImportAndMatching:
    ...
    def test_import_samples_1381(self, comprehensive_db, sample_data_dir, matching_config):
        ...
        with BookContext(book_name, db_url) as ctx:
            ...
            matched_count = 0
            imported_count = 0
            for action, txn in matching_service.match_transactions(
                import_account, transactions_to_import, candidates
            ):
                if action == 'match':
...
class TestDuplicateDetection:
    ...
    def test_no_true_duplicates(self, comprehensive_db):
        ...
        with BookContext(book_name, db_url) as ctx:
            ...
            for txn in transactions:
                splits_signature = tuple(
                    sorted([(split.account_id, split.amount) for split in txn.splits])
                )
                signature = (
            ...
            duplicates = [
                (sig, txns) for sig, txns in transaction_signatures.items() if len(txns) > 1
            ]
