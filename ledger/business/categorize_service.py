...
class CategoryRules:
    ...
    def _load_rules(self):
        ...
        try:
            ...
            logger.debug(f"_load_rules: file not found, no rules loaded")
            self.rules = {}
            return
        ...
        logger.debug(f"_load_rules: loaded {len(self._compiled_patterns)} categories with {total_patterns} patterns")
    ...
    def match(self, payee_norm: str) -> str | None:
        ...
        logger.debug(f"match: searching {len(self._compiled_patterns)} categories for '{payee_norm}'")
        
        # Check all patterns
        for category, patterns in self._compiled_patterns.items():
            for pattern, cat in patterns:
                if pattern.search(payee_norm):
                    logger.debug(f"match: FOUND - pattern '{pattern.pattern}' matched, category='{cat}'")
                    return cat
...
class CategorizeService:
    ...
    def lookup_category_for_payee(
        ...
        if cache_entry:
            logger.debug(f"lookup_category_for_payee: cache HIT - account_id={cache_entry.account_id}")
            ...
            try:
                ...
                logger.debug(f"lookup_category_for_payee: returning cached category '{category_account.full_name}'")
                return (category_account.full_name, 'cache')
            except Exception as e:
                # Account not found, continue to tier 2
                logger.debug(f"lookup_category_for_payee: cache entry invalid (account not found: {e}), continuing to Tier 2")
        else:
        ...
        if matched_category:
            ...
            try:
                ...
                if update_cache:
                    ...
                    logger.debug(f"lookup_category_for_payee: cached mapping '{payee_norm}' -> '{matched_category}'")
                logger.debug(f"lookup_category_for_payee: returning rule-matched category '{matched_category}'")
                return (matched_category, 'rule')
            except Exception as e:
                warning(f"Category {matched_category} found for payee {payee_norm} but account not found")
                logger.debug(f"lookup_category_for_payee: account '{matched_category}' not found: {e}")

        # Tier 3: No match - return None, let caller handle fallback
        logger.debug(f"lookup_category_for_payee: Tier 3 - no match found for '{payee_norm}', returning None")
        return None
